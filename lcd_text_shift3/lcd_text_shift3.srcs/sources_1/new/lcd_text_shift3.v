	...
	if(!rst) begin
		...
		case(state)
			...
			DELAY_T : begin
				...
				if(cnt == 50) state = DISP_SHIFT;//쉬프트 속도 조절 약 0.5sec
			end
			DISP_SHIFT : begin
		...
		case(state)
			...
			LINE2: begin
				case(cnt)
					...
					12 : {LCD_RS, LCD_RW, LCD_DATA} = 10'b1_0_0100_0001; //A
					13 : {LCD_RS, LCD_RW, LCD_DATA} = 10'b1_0_0100_0010; //B
					14 : {LCD_RS, LCD_RW, LCD_DATA} = 10'b1_0_0010_0000; //
					15 : {LCD_RS, LCD_RW, LCD_DATA} = 10'b1_0_0010_0000; //
					16 : {LCD_RS, LCD_RW, LCD_DATA} = 10'b1_0_0010_0000; //
					default : {LCD_RS, LCD_RW, LCD_DATA} = 10'b1_0_0010_0000; //
...
assign LCD_E = clk; //Enable -> read/write 를 하고자 할 때 E가 high(1) 에서 low(0) 로 떨어지는 순간에 데이터가 data pins 에 보내짐
